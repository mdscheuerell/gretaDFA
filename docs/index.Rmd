...
mcmc_smpl <- mcmc(mod_fit, n_samples = 10000, thin = 5, warmup = 5000,
                  chains = 1, verbose = FALSE,
                  hmc(Lmin = 10, Lmax = 100, epsilon = 0.1, diag_sd = 1))
mcmc_smpl <- extra_samples(mcmc_smpl, n_samples = 10000, thin = 5, verbose = FALSE)
```
...
par(mfrow=c(NN,MM), mai=rep(0,4), omi=rep(0,4))
for(i in 1:NN) {
	for(j in 1:MM) {
