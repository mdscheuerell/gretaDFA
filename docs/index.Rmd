...
To make the model identifiable when $M$ > 1, we need to impose several constraints on the model. First, the upper right section of $\mathbf{Z}$ must be set equal to zero (i.e., $z_{ij} = 0 ~ \forall ~ i < j$). For example, if $M$ = 3 then 
...
# ZZ_est_raw_d = normal(0, 10, MM, truncation = c(0,1))
...
```{r priors_obs_cov}
## diagonal of R
# RR_est_raw = student(10, 0, 1, truncation=c(0, 1))
# RR_est_raw = cauchy(0, 5, 1, truncation=c(0, 1))
RR_est_raw = inverse_gamma(1, 3, 1, truncation=c(0, 1))
```
...
mod_fit <- model(xx_est, ZZ_est, RR_est_raw, sigma)
mcmc_smpl <- mcmc(mod_fit, n_samples = 10000, thin = 10, warmup = 5000,
                  chains = 1, verbose = FALSE,
                  hmc(Lmin = 10, Lmax = 100, epsilon = 0.1, diag_sd = 1))
```
