...
We begin by loading __greta__ and defining a function that returns the cumulative sum of the rows of a matrix that we will need for estimating the latent states $(\mathbf{x})$.

```{r load_greta, message=FALSE}
library(greta)
op <- greta::.internals$nodes$constructors$op
## cumsums across matrix rows
...
```{r func_test}
testmat <- normal(0, 1, dim = c(2, 3))
y_row <- cumsum_row(testmat)
evaluate(y_row, list(testmat = matrix(1, 2, 3)))
```
...
```{r like_obs, eval=FALSE}
CC <- chol(RR_est)
vv <- normal(0, 1, NN)
yy <- ZZ_est %*% xx_est + CC %*% vv
```
