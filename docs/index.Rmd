...
Next I'll simulate the latent trends. In this case, each of them is random walk, so it's easy to just work with the innovations. To do so, I make use of `mvrnorm` function in the `MASS` package. I also make use of the color palettes in `viridis`. I will also discard the first `r TT` samples in order to eliminate the effects of the initial starting conditions.
...
```{r create_Z}
ZZ <- matrix(NA, NN, MM)
LL <- seq(0,0.8,length.out=6)
for(i in 1:MM) {
  zz <- NULL
  for(j in 1:(MM)) {
    zz <- c(zz,runif(NN/MM,LL[j],LL[j+1]))
  }
  ZZ[sample(NN,NN),i] <- zz
}
```
...
matplot(t(yy), type="l", lty="solid",
        ...
        col=plasma(NN, end=0.8))
```

This collection of `r NN` time series appears to be highly correlated. Here is a histogram of the correlation coefficients. 

```{r hist_cor_coefs, fig.height=4, fig.width=7}
par(mai=c(0.8,0.8,0.2,0.2))
hist(cor(yy)[lower.tri(yy)], breaks=seq(-10,10)/10,
     xlab="Correlation", ylab="", main="")
```

## Parameter estimation

In addition, to make the model identifiable when $M > 1$, the upper right section of $\mathbf{Z}$ must be set equal to zero. For example, if $M = 3$, then 
