...
The first thing to do is specify the prior distributions for the model parameters. Defining the priors for the covariance matrix $\mathbf{R}$ is straightforward. We'll use a relatively uninformative half-cauchy prior on the variances with a scale of 25 and truncate the upper end at 100.

```{r priors_obs_cov}
## cov matrix of obs errors
RR_est <- as_data(diag(NN))
diag(RR_est) <- cauchy(0, 5, NN, truncation=c(0, 20))
```
...
Here is the likelihood for the data.
...
dim(Zx_vec) <- c(NN*TT, 1)
## create expanded cov matrix
RR_vec <- as_data(diag(NN*TT))
diag(RR_vec) <- rep(diag(RR_est),TT)
## define likelihood
...
mod_fit <- model(xx_est, ZZ_est, RR_est)
mcmc_smpl <- mcmc(mod_fit, n_samples = 100, thin = 1, warmup = 100,
