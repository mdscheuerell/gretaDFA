...
mcmc_smpl <- mcmc(mod_fit, n_samples = 300, thin = 1, warmup = 400,
...
Here are the `r MM` factors $\mathbf{x}$. Note that we need to rotate these just as we did $\mathbf{Z}$.
...
par(mai=c(0.8,0.8,0.2,0.2))
matplot(t(xx_rot), type="b", lty="solid", cex=0.7,
        xlab="Time", ylab=expression(italic(x)[italic(t)]),
        col=plasma(MM, end=0.8))
```
