...
__WARNING__: This vignette is still in the testing and evaluating phase and should not be considered complete or error-free.
...
Next we'll simulate the latent trends. In this case, each of them is random walk, so it's easy to just work with the innovations. To do so, we make use of `mvrnorm()` function in the __MASS__ package. We also make use of the color palettes in the __viridis__ package.
...
Now we can use the loadings matrix and some observation errors to create the observed time series. Here we'll assume that the errors are IID, and their variance is 0.2. For now we will ignore the additive effect of the offset vector $\mathbf{a}$. (We note here, too, that one could always subtract the mean of each of the $N$ time errors from $\mathbf{y}$ and thereby avoid having to estimate $\mathbf{a}$.)
...
We begin by loading the __tensorflow__ and __greta__ packages, and defining a function that returns the cumulative sum of the rows of a matrix that we will use for estimating the latent states $(\mathbf{x})$.
...
The first thing to do is specify the prior distributions for the model parameters. 
...
Defining the priors for the covariance matrix $\mathbf{R}$ is straightforward. We'll use a relatively uninformative half-cauchy prior on the variances with a scale of 25 and truncate the upper end at 100.

```{r priors_obs_cov}
## empty R
RR_est <- zeros(NN,NN)
## diagonal of R
RR_est_raw <- cauchy(0, 1, NN, truncation=c(0, 5))
diag(RR_est) <- RR_est_raw
```
...
## vectorize mean
#Zx_vec <- ZZ_est %*% xx_est
Zx_vec <- c(ZZ %*% xx_est)
## create expanded cov matrix
RR_vec <- zeros(NN*TT, NN*TT)
diag(RR_vec) <- rep(RR_est_raw, TT)
## define likelihood
distribution(yy_vec) = multivariate_normal(Zx_vec, RR_vec)
```
...
mod_fit <- model(xx_est, RR_est_raw)
mcmc_smpl <- mcmc(mod_fit, n_samples = 300, thin = 1, warmup = 500,
...
fff <- mod_smry$statistics[,"Mean"]
```

Here are the `r MM` factors $\mathbf{x}$.

```{r get_factors}
xx_fit <- fff[grepl("xx",rownames(mod_smry$statistics))]
xx_fit <- matrix(xx_fit, MM, TT, byrow=FALSE)
#xx_rot <- solve(HH_inv) %*% xx_fit
```


```{r plot_xx_fits, fig.height=4, fig.width=7, echo=FALSE}
par(mai=c(0.8,0.8,0.2,0.2))
...
and the loadings matrix $\mathbf{Z}$.

```{r get_ZZ_fit}

#ZZ_fit <- fff[grepl("ZZ",rownames(mod_smry$statistics))]
...
```{r get_RR_fit}
RR_fit <- fff[grepl("RR",rownames(mod_smry$statistics))]
RR_fit <- diag(RR_fit)
round(RR_fit, 2)
```
...
par(mai=c(0.8,0.8,0.2,0.2))
matplot(t(yy_fit), type="l", lty="solid",
        xlab="Time", ylab=expression(italic(y)[italic(t)]),
        col=plasma(NN, alpha=0.7, end=0.8))
```
