---
title: "Using the `greta` package to do Dynamic Factor Analysis"
output:
  html_document:
    theme: cerulean
    toc: true
...
```{r sim_trends, fig.height=4, fig.width=7}
library(MASS)
library(viridis)
set.seed(123)
...
xx <- apply(ww,1,cumsum)
## plot the trends
par(mai=c(0.8,0.8,0.2,0.2))
matplot(xx, type="b", lty="solid", cex=0.8,
...
The next thing to do is create the matrix $\mathbf{Z}$ that maps the trends onto the observations. I want some of the time series to look more like one another than others, so I use a range of values within some blocks.
