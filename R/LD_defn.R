...
ld <- function (sigma, N = 2L, dim = 1L)
  distrib("ld", sigma, N, dim)
...
ld_distribution <- R6Class (
  ...
  public = list(
    ...
    initialize = function (sigma, N, dim) {
      
      # sigma <- as.greta_array(sigma)
      # N <- as.greta_array(sigma)
    ...
    tf_distrib = function (parameters, dag) {
      ...
      # const <- fl(n) - tf$lin_space(1, n, n)
      # const <- tf$lin_space(1, n, n)

      log_prob = function (x) {
        const <- tf$fill(n, N) #- tf$lin_space(1, n, n)
        # const * tf$log(x) - x ^ fl(2) / (fl(2) * sigma)
        # tf$log(x) - x ^ fl(2) / (fl(2) * sigma)
      }
...
z <- ld(sigma = 1, N = 3, dim = 1)
m <- model(z)

# plan(multicore)

draws <- mcmc(m, chains = 2, n_samples = 1000, warmup = 500, thin = 4)
plot(draws)
